...
function Card({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot='card'
      className={cn(
        'flex min-h-0 w-full max-w-full flex-col overflow-hidden rounded-md border bg-card text-card-foreground',
        className
      )}
      {...props}
    />
...
function CardContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('min-h-0 min-w-0 overflow-auto p-6', className)}
      {...props}
    />
  )
}
