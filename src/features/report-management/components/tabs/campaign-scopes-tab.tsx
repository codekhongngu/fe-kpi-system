...
import { formManagementApi, type OrgTreeItem } from '../../../form-management/api/template-management-api'
...
export function CampaignScopesTab({ campaignId, templateId }: CampaignScopesTabProps) {
  ...
  useEffect(() => {
    ...
    if (!selectedOrgId || !flatOrgs.some((item) => item.id === selectedOrgId && item.canAssignReports)) {
  ...
  const availableIndicators = useMemo(() => {
    ...
    return flatIndicators.filter((indicator) => {
      if (indicator.type === 'INPUT' && selectedOrgIndicatorIds.has(indicator.id)) return false
      if (keyword) {
  ...
  const totalInputIndicators = indicators.filter(i => i.type === 'INPUT').length
  const assignedInputIndicatorsCount = new Set(
    rows.map(r => r.indicatorId).filter(id => {
      const ind = indicators.find(i => i.id === id)
      return ind && ind.type === 'INPUT'
    })
  ).size
  const progressPercent = totalInputIndicators ? Math.round((assignedInputIndicatorsCount / totalInputIndicators) * 100) : 0
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const initial = dedupeScopes(scopesQuery.data ?? [])
      const current = dedupeScopes(rows).filter((item) => item.orgId.trim() && item.indicatorId.trim())
      
      const initialKeys = new Set(initial.map(s => `${s.orgId}::${s.indicatorId}`))
      const currentKeys = new Set(current.map(s => `${s.orgId}::${s.indicatorId}`))
      
      const added = current.filter(s => !initialKeys.has(`${s.orgId}::${s.indicatorId}`))
      const removed = initial.filter(s => !currentKeys.has(`${s.orgId}::${s.indicatorId}`))
      
      if (added.length > 0) {
        await reportCampaignApi.upsertScopes(campaignId, added.map(i => ({ orgId: i.orgId, indicatorId: i.indicatorId })))
      }
      if (removed.length > 0) {
        await reportCampaignApi.deleteScopes(campaignId, removed.map(i => ({ orgId: i.orgId, indicatorId: i.indicatorId })))
      }
    },
    onSuccess: async () => {
      toast.success('Đã lưu phạm vi phân bổ chỉ tiêu.')
      await queryClient.invalidateQueries({ queryKey: ['report-management', 'campaign', campaignId, 'scopes'] })
    },
    onError: (error: Error) => toast.error(error.message),
  })

  const snapshotMutation = useMutation({
    mutationFn: () => apiClient.get<{ items: { orgId: string; indicatorId: string }[] }>(`/forms/${templateId}/template-scopes`),
    onSuccess: (response) => {
      ...
      setRows(dedupeScopes(templateScopes.map(s => ({ orgId: s.orgId, indicatorId: s.indicatorId }))))
      toast.success(`Đã lấy snapshot từ template (${templateScopes.length} bản ghi). Hãy nhấn Lưu để xác nhận.`)
    },
  ...
  function getAvailableDescendantInputs(node: IndicatorTreeNode): string[] {
    let ids: string[] = []
    if (node.type === 'INPUT') {
      const assignedToOther = isUnique && orgByIndicatorId.has(node.id) && orgByIndicatorId.get(node.id) !== selectedOrgId
      if (availableIndicators.some(i => i.id === node.id) && !assignedToOther) {
        ids.push(node.id)
      }
    }
  ...
  function getAssignedDescendantInputs(node: IndicatorTreeNode): string[] {
    let ids: string[] = []
    if (node.type === 'INPUT') {
      if (assignedIndicators.some(i => i.id === node.id) && selectedOrgIndicatorIds.has(node.id)) {
        ids.push(node.id)
      }
    }
    for (const child of node.children) {
      ...
      <CardHeader className='gap-4'>
        <div className='flex flex-wrap items-start justify-between gap-3'>
          <div>
            ...
            <CardDescription>Phân bổ chỉ tiêu cho các đơn vị tham gia báo cáo.</CardDescription>
          </div>
          <div className='flex flex-wrap gap-2'>
            <Button
              type='button'
              variant='outline'
              className='gap-2 rounded-xl text-xs font-bold px-3'
            ...
            >
              <RefreshCcw className={snapshotMutation.isPending ? 'size-4 animate-spin' : 'size-4'} />
      ...
      <CardContent className={contentClass}>
        <div className='mb-6 rounded-2xl border bg-muted/20 p-4'>
          <div className='flex justify-between items-center mb-2'>
            <span className='text-sm font-bold text-foreground'>Tiến độ phân bổ chỉ tiêu</span>
            <span className={`text-sm font-bold ${progressPercent === 100 ? 'text-primary' : 'text-amber-500'}`}>
              {assignedInputIndicatorsCount} / {totalInputIndicators} ({progressPercent}%)
            </span>
          </div>
          ...
          {progressPercent < 100 && (
            <p className='mt-2 text-xs text-amber-600 flex items-center gap-1'>
              ...
              Cần phân bổ 100% chỉ tiêu INPUT trước khi có thể chuyển biểu mẫu sang trạng thái Sẵn sàng.
            </p>
          )}
        </div>
                          ...
                          <button
                            ...
                            className={[
                              ...
                              isSelected
                                ...
                                : canAssign ? 'hover:bg-muted/50 font-medium text-foreground' : 'opacity-40 cursor-not-allowed',
                            ].join(' ')}
                            onClick={() => {
                              if (canAssign) setSelectedOrgId(item.id)
                            }}
                          >
                            <div style={{ paddingLeft: `${(item.level - 1) * 12}px` }} className='flex items-center gap-2 overflow-hidden'>
                              <Building2 className={isSelected ? 'size-4 text-primary shrink-0' : 'size-4 text-muted-foreground shrink-0'} />
                              <span className='min-w-0 truncate'>{item.code} - {item.name}</span>
                            </div>
                          </button>
                        )
                      })}
              ...
              <div className='flex items-center gap-4'>
                ...
                <div>
                  ...
                  <div className='mt-0.5 text-lg font-bold text-foreground'>
                    {selectedOrgSummary ? `${selectedOrgSummary.code} - ${selectedOrgSummary.name}` : 'Chọn đơn vị để bắt đầu'}
                  </div>
                ...
                <CardContent className='p-0 max-h-[500px] overflow-auto'>
                  <Table>
                    ...
                    <TableBody>
                      {!selectedOrgId ? (
                        <TableRow>
                          <TableCell colSpan={2} className='py-12 text-center text-xs text-muted-foreground'>
                        ...
                        availableIndicators.map((item) => {
                          ...
                          const assignedToOtherOrg = isUnique && isInput && orgByIndicatorId.has(item.id) && orgByIndicatorId.get(item.id) !== selectedOrgId
                          const otherOrgId = assignedToOtherOrg ? orgByIndicatorId.get(item.id) : null
                          const otherOrgName = otherOrgId ? flatOrgs.find(o => o.id === otherOrgId)?.name : null
                          
                          const descendantInputIds = getAvailableDescendantInputs(item)
                          const canCheck = isInput ? !assignedToOtherOrg : descendantInputIds.length > 0
                          const allSelected = descendantInputIds.length > 0 && descendantInputIds.every(id => selectedAvailableIds.includes(id))
                          const someSelected = descendantInputIds.length > 0 && descendantInputIds.some(id => selectedAvailableIds.includes(id))
                          
                          const isChecked = isInput ? selectedAvailableIds.includes(item.id) : allSelected
                            ...
                            <TableRow key={item.id} className='hover:bg-muted/10'>
                              <TableCell className='w-12 text-center'>
                                <div className='flex justify-center'>
                                  <Checkbox
                                    checked={isInput ? isChecked : allSelected ? true : someSelected ? 'indeterminate' : false}
                                    onCheckedChange={(checked) => {
                                      if (isInput) {
                                        setSelectedAvailableIds(prev => checked ? [...prev, item.id] : prev.filter(id => id !== item.id))
                                      } else {
                                        setSelectedAvailableIds(prev => checked 
                                          ? Array.from(new Set([...prev, ...descendantInputIds]))
                                          : prev.filter(id => !descendantInputIds.includes(id))
                                        )
                                      }
                                    }}
                              ...
                              <TableCell>
                                <div style={{ paddingLeft: `${((item.depth || 1) - 1) * 16}px` }} className='flex flex-col gap-0.5'>
                                  <div className={`text-xs ${isInput ? 'font-medium' : 'font-bold'} flex items-center gap-2`}>
                                    {item.code} - {item.name}
                                    {item.type === 'TITLE' && <Badge variant='outline' className='text-[9px] py-0 px-1'>Nhóm</Badge>}
                                  </div>
                                  {assignedToOtherOrg && (
                                    <div className='text-[10px] text-destructive flex items-center gap-1 mt-1'>
                                      <AlertCircle className='size-3' /> Đã giao cho: {otherOrgName || otherOrgId}
                                    </div>
                                  )}
                                </div>
                              </TableCell>
                    ...
                    <TableBody>
                      {!selectedOrgId ? (
                        ...
                        assignedIndicators.map((item) => {
                          const isInput = item.type === 'INPUT'
                          const descendantInputIds = getAssignedDescendantInputs(item)
                          const canCheck = isInput ? true : descendantInputIds.length > 0
                          const allSelected = descendantInputIds.length > 0 && descendantInputIds.every(id => selectedAssignedIds.includes(id))
                          const someSelected = descendantInputIds.length > 0 && descendantInputIds.some(id => selectedAssignedIds.includes(id))
                          
                          const isChecked = isInput ? selectedAssignedIds.includes(item.id) : allSelected
                          ...
                          return (
                            <TableRow key={item.id} className='hover:bg-muted/10'>
                              <TableCell className='w-12 text-center'>
                                <div className='flex justify-center'>
                                  <Checkbox
                                    ...
                                    onCheckedChange={(checked) => {
                                      if (isInput) {
                                        setSelectedAssignedIds(prev => checked ? [...prev, item.id] : prev.filter(id => id !== item.id))
                                      } else {
                                        setSelectedAssignedIds(prev => checked 
                                          ? Array.from(new Set([...prev, ...descendantInputIds]))
                                          : prev.filter(id => !descendantInputIds.includes(id))
                                        )
                                      }
                              ...
                              <TableCell>
                                <div style={{ paddingLeft: `${((item.depth || 1) - 1) * 16}px` }} className='flex flex-col gap-0.5'>
                                  <div className={`text-xs ${isInput ? 'font-medium' : 'font-bold'}`}>
                                    {item.code} - {item.name}
                                    {item.type === 'TITLE' && <Badge variant='outline' className='ml-2 text-[9px] py-0 px-1'>Nhóm</Badge>}
                                  </div>
                                </div>
                              </TableCell>
                            </TableRow>
