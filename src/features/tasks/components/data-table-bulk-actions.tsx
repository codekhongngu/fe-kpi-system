...
export function DataTableBulkActions<TData>({
  ...
  const handleBulkStatusChange = (status: string) => {
    ...
    toast.promise(sleep(2000), {
      ...
      success: () => {
        ...
        return `Đã cập nhật trạng thái "${status}" cho ${selectedTasks.length} công việc.`
      },
      error: 'Có lỗi xảy ra',
    })
    table.resetRowSelection()
  ...
  const handleBulkPriorityChange = (priority: string) => {
    const selectedTasks = selectedRows.map((row) => row.original as Task)
    toast.promise(sleep(2000), {
      loading: 'Đang cập nhật mức ưu tiên...',
      success: () => {
        table.resetRowSelection()
        return `Đã cập nhật mức ưu tiên "${priority}" cho ${selectedTasks.length} công việc.`
      },
      error: 'Có lỗi xảy ra',
    })
    table.resetRowSelection()
  ...
  const handleBulkExport = () => {
    const selectedTasks = selectedRows.map((row) => row.original as Task)
    toast.promise(sleep(2000), {
      loading: 'Đang xuất dữ liệu công việc...',
      success: () => {
        table.resetRowSelection()
  ...
  return (
    <>
      <BulkActionsToolbar table={table} entityName='công việc'>
        <DropdownMenu>
          <Tooltip>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <DropdownMenuTrigger asChild>
                ...
                >
                  <ArrowUpDown />
                  <span className='sr-only'>Cập nhật mức ưu tiên</span>
                </Button>
              </DropdownMenuTrigger>
            </TooltipTrigger>
            <TooltipContent>
              <p>Cập nhật mức ưu tiên</p>
            </TooltipContent>
          </Tooltip>
          <DropdownMenuContent sideOffset={14}>
